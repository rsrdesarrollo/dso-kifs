...
void usage(int argc,char* argv[]){
	printf("Usage: %s [-r|-w|-d] <entry_name> [value]\n",argv[0]);
}
...
int main(int argc, char *argv[])
{
	...
	/* Loop to process options */
        while ((optc = getopt(argc, argv, "+hrwd")) != -1) {
                switch (optc) {
                case 'r':
	...
	if (kifs_op==KIFS_WRITE_OP)
	{
		...
		*/
		...
		printf("%s\n",buf);
		printf("*****\n");	
	}else if (kifs_op==KIFS_DESC_OP){

		nchars=USER_BUFFER_MAX;
		ret=kifs(entryname,KIFS_DESC_OP,buf,nchars);
